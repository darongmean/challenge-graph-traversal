...
(defn G [n s]
  {:pre [(<= (- n 1) s) (< s (/ (* n (- n 1)) 2))]}
  (let [vertices (->> (range 1 (inc n)) (map str) (map keyword))
        base-graph (reduce #(assoc %1 %2 {}) {} (shuffle vertices))
        ...
        other-combinator-edges (for [head (shuffle vertices)
                                 ...
                                 [head tail])
        graph-edges (concat connected-graph-min-edges (take (- s (- n 1)) other-combinator-edges))]
    (reduce (fn [graph [head tail]]
              (assoc-in graph [head tail] (inc (count (get graph head)))))
            base-graph graph-edges)))
